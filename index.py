...
class Game:
    ...
    def describe_down(self, args):
        ...
        print "<p>Down: {:d} Distance: {:.0f} Yardline: {:s}</p>".format( self.scoreboard.down, self.scoreboard.distance, yard)
    ...
    def field_goal(self, args):
        print "<p>{:.0f} Yard Field Goal!</p>".format((100 - self.scoreboard.yardline) + 7)
        value = R.randrange(0, 1000)
        made = self.offense.field_goal_chance()
        print "Field Goal Chance: {:.0f}".format(made)
        if value < made:
            print "<p>It's good!</p>"
...
if __name__ == "__main__":
    ...
    game.fsm.set_start("Check Yardage")
    game.fsm.run("")
